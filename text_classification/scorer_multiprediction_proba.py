...
"""this function is for multi predictions with one true label. Only compute for classes with probability > threshold
...
def get_result_output(stats):
    ...
    labels = dict(stats)
    del labels['overall']
    for i in sorted(labels.items(), key=lambda x: x[1][0], reverse=True):   # sort by precision@1
        tag = i[0]
...
def score(args):
    gold_tmp = [line.replace('__label__', '') for line in get_lines(args.gold)]
    # remove prediction if probability < threshold
    system_tmp = [line for line in get_lines(args.system)]
    system_tmp = [[x.replace('__label__', '').split(' ') for x in a.split(' __label__')] for a in system_tmp]
    gold, system = [], []
    for i, v in enumerate(system_tmp):
        if any([float(x[1]) > args.threshold for x in v]) :       # if any prediction > threshold, append all predictions
            predictions = [a[0] for a in v]
            system.append(predictions)
...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=0.4, help='threshold for probability to be considered')
